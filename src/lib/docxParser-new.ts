...
export async function parseDocx(file: File): Promise<ParsedQuestion[]> {
  ...
  const lines = html
    .replace(/<\/?p[^>]*>|<br\s*\/?>|<\/?li[^>]*>/gi, "\n") // p, br və li teqlərini sətir sonu kimi qəbul edirik
    .replace(/<[^>]+>/g, "") // variant hərflərinə mane olan digər bütün HTML teqlərini (məsələn: <b>, <span>) silirik
  ...
  while (i < lines.length) {
    ...
    if (qMatch) {
      ...
      while (i < lines.length) {
        if (genericPattern.test(lines[i])) {
            ...
            for (let j = blockLines.length - 1; j >= 0; j--) {
                if (/^\s*[A-Ea-e][.):\-]\s*/.test(blockLines[j])) {
                    variantCount++;
                } else {
                    break;
                }
      ...
      const fullText = [questionHeader, ...textPart].join("\n");

      const options = optionPart.map(opt => opt.replace(/^\s*[A-Ea-e][.):\-]?\s*/, "").trim());
