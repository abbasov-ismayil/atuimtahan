...
import { FileText, MessageSquare, Send, ChevronDown, ChevronRight, Ticket } from "lucide-react";
...
function ChatPanel({ examResultId, examName, userId }: { examResultId: string; examName: string; userId: string }) {
  ...
  const sendMessage = async () => {
    ...
    const { error } = await supabase.from("messages" as any).insert([{
      sender_id: userId, exam_result_id: examResultId,
      content: newMsg.trim(), is_from_admin: false,
    }]);
  ...
  return (
    <div className="mt-3 rounded-xl border border-border bg-muted/20 p-3">
      <p className="text-[10px] text-muted-foreground mb-2">📝 {examName}</p>
        ...
        {messages.length === 0 ? (
          ...
          messages.map((m) => (
            <div key={m.id} className={`text-xs p-2 rounded-lg max-w-[80%] ${
              m.is_from_admin ? "bg-primary/10 text-foreground ml-0" : "bg-accent text-foreground ml-auto"
            }`}>
...
export default function MyExamsPage() {
  ...
  const ticketResults = results.filter(r => r.exam_type === "ticket");

  return (
    <div className="container max-w-2xl py-12">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-3xl font-bold text-foreground mb-2">İmtahanlarım</h1>
        <p className="text-muted-foreground mb-8">Test və bilet imtahanlarınızın nəticələrini izləyin.</p>
        ...
        {loading ? (
          ...
          <div className="p-6 rounded-2xl border border-border bg-muted/20 text-center">
            <p className="text-sm text-muted-foreground">Hələ imtahan nəticəniz yoxdur.</p>
          </div>
        ) : (
          <div className="space-y-6">
            {testResults.length > 0 && (
              <div>
                <h2 className="text-sm font-bold text-foreground mb-3 flex items-center gap-2">
                  <FileText className="h-4 w-4 text-primary" /> Test İmtahanları
                </h2>
                <div className="space-y-3">
                  {testResults.map((r) => (
                    <ExamCard key={r.id} r={r} expandedExam={expandedExam} setExpandedExam={setExpandedExam} userId={user?.id} />
                  ))}
                </div>
              </div>
            )}
            ...
            {ticketResults.length > 0 && (
              <div>
                <h2 className="text-sm font-bold text-foreground mb-3 flex items-center gap-2">
                  <Ticket className="h-4 w-4 text-purple-600" /> Bilet İmtahanları
                </h2>
                <div className="space-y-3">
                  {ticketResults.map((r) => (
                    <ExamCard key={r.id} r={r} expandedExam={expandedExam} setExpandedExam={setExpandedExam} userId={user?.id} />
...
function ExamCard({ r, expandedExam, setExpandedExam, userId }: {
  r: ExamResult; expandedExam: string | null; setExpandedExam: (id: string | null) => void; userId?: string;
}) {
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden">
      <div className="p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-foreground">{r.exam_name || "Adsız imtahan"}</p>
      ...
      {expandedExam === r.id && userId && (
        <div className="px-4 pb-4">
          <ChatPanel examResultId={r.id} examName={r.exam_name || "Adsız imtahan"} userId={userId} />
        </div>
      )}
    </div>
  );
